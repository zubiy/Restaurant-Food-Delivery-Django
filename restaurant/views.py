...
from .forms import RestuarantSignUpForm,RestuarantForm
...
def Logout(request):
	if request.user.is_active:
		logout(request)
		return redirect("restuarant:rlogin")
	else:
		logout(request)
		return redirect("restuarant:login")
...
def restuarantMenu(request,pk=None):
	...
	items =[]
	for i in menu:
		item = Product.objects.filter(name=i.item_id)
		for content in item:
			temp=[]
			temp.append(content.name)
...
def restRegister(request):
	...
	if form.is_valid():
		...
		if user is not None:
			if user.is_active:
				login(request,user)
				return redirect("restuarant:rcreate")
	context ={
		'form':form
	}			
...
def restLogin(request):
	if request.method=="POST":
		...
		if user is not None:
			if user.is_active:
				login(request,user)
				return redirect("restuarant:rprofile")
			else:
...
def change_password(request):
    if request.method == 'POST':
        form = PasswordChangeForm(request.POST or None,request.FILES or None, request.user)
        if form.is_valid():
            user = form.save()
            update_session_auth_hash(request, user)
            messages.success(request, 'Your password was successfully updated!')
            return HttpResponseRedirect(reverse('restuarant:restaurantProfile'))
    else:
...
def menuManipulation(request):
	...
	if request.POST:
		...
		if type =="Modify":
			...
			item=Product.objects.filter(id=itemid)
			item=item[0]
			menu=Menu()
			menu.item_id=item
			menu.r_id=rest
